...
async function verifyToken(authHeader) {
  ...
  const googleAudience = process.env.GOOGLE_CLIENT_ID ?? process.env.GOOGLE_IOS_CLIENT_ID ?? process.env.GOOGLE_ANDROID_CLIENT_ID ?? process.env.GOOGLE_WEB_CLIENT_ID;
  let iss;
  try {
  ...
  console.log("[auth] verifying token", {
    ...
    expectedGoogleAud: googleAudience,
...
async function uploadBuffer(key, buffer, contentType) {
  await s3.send(
    new import_client_s3.PutObjectCommand({ Bucket: BUCKET, Key: key, Body: buffer, ContentType: contentType })
  );
}
...
var handler = async (event) => {
  ...
  if (pdfBase64) {
    ...
    message2 = { jobId, mode, pdfKey, title };
  } else if (url) {
    message2 = { jobId, mode, url };
  } else {
    message2 = { jobId, mode, text, title };
  }
  await sqs.send(
