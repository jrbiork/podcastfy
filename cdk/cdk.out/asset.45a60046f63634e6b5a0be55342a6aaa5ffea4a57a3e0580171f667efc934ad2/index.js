...
function pdfInputKey(jobId) {
  return `jobs/${jobId}/input.pdf`;
}
var PDF_PUT_EXPIRES_SEC = 900;
async function getPresignedPdfPutUrl(jobId) {
  ...
  const uploadUrl = await (0, import_s3_request_presigner.getSignedUrl)(s3, cmd, { expiresIn: PDF_PUT_EXPIRES_SEC });
  return { uploadUrl, pdfKey };
}
async function headPdfInput(jobId) {
  const res = await s3.send(
    new import_client_s3.HeadObjectCommand({ Bucket: BUCKET, Key: pdfInputKey(jobId) })
  );
  return {
...
async function readStatus(jobId) {
  try {
    const res = await s3.send(
      new import_client_s3.GetObjectCommand({ Bucket: BUCKET, Key: statusKey(jobId) })
    );
...
var VALID_VOICES = ["alloy", "echo", "fable", "nova", "onyx", "shimmer"];
var VALID_LANGS = ["en-US", "en-GB", "es", "fr", "de", "pt-BR"];
...
function parseVoiceLanguage(body) {
...
async function handlePdfPresign(requestId, _body) {
...
async function handlePdfFinalize(requestId, body) {
  ...
  await sqs.send(
    new import_client_sqs.SendMessageCommand({ QueueUrl: QUEUE_URL, MessageBody: JSON.stringify(message2) })
  );
  console.log("[dispatcher] job enqueued after pdf finalize", { requestId, jobId, mode });
  return json(200, { jobId, status: "queued" });
}
async function handleDispatchJob(requestId, body) {
  const mode = body.mode === "tts" ? "tts" : "podcast";
  ...
  let message2;
  if (url) {
    message2 = { jobId, mode, url, ...voice ? { voice } : {}, ...language ? { language } : {} };
  } else {
    message2 = {
      jobId,
      mode,
