...
var appleJwks = createRemoteJWKSet(
  new URL("https://appleid.apple.com/auth/keys")
);
var AuthError = class extends Error {
  constructor(message2, statusCode = 401) {
    super(message2);
...
async function verifyToken(authHeader) {
  ...
  console.log("[auth] verifying token", {
    ...
    aud: payload.aud,
    exp: payload.exp,
    expDelta: payload.exp ? payload.exp - Math.floor(Date.now() / 1e3) : null,
...
var TRIGGER_LAMBDA_ARN = process.env.TRIGGER_LAMBDA_ARN;
var SCHEDULER_ROLE_ARN = process.env.SCHEDULER_ROLE_ARN;
var SCHEDULE_GROUP = process.env.SCHEDULE_GROUP ?? "podcastify-digest-schedules";
var SNS_PLATFORM_APPLICATION_ARN = process.env.SNS_PLATFORM_APPLICATION_ARN;
...
function pathEndsWith(rawPath, suffix) {
  return (rawPath ?? "").endsWith(suffix);
}
function readExistingPrefs(item) {
  if (!item) {
    return {
      feedUrls: null,
      selectedTopics: null,
  ...
  return {
    topicFeedUrls: item.topicFeedUrls?.M ? Object.fromEntries(
    ...
    durationMinutes: item.durationMinutes?.N != null ? Number(item.durationMinutes.N) : null,
    deliveryHour: item.deliveryHour?.N != null ? Number(item.deliveryHour.N) : 6,
...
async function handleGet(userId) {
  try {
    ...
    return json(200, {
      ...
      topicFeedUrls: result.Item.topicFeedUrls?.M ? Object.fromEntries(
        Object.entries(result.Item.topicFeedUrls.M).map(
          ([topicId, urlsAttr]) => [
            topicId,
            (urlsAttr.L ?? []).map((v) => v.S).filter(
              (u) => typeof u === "string" && u.length > 0
            )
          ]
        )
      ) : null,
      deliveryHour: result.Item.deliveryHour?.N != null ? Number(result.Item.deliveryHour.N) : null,
      selectedTopics: result.Item.selectedTopics?.SS ?? null,
      voice: result.Item.voice?.S ?? null,
      durationMinutes: result.Item.durationMinutes?.N != null ? Number(result.Item.durationMinutes.N) : null,
...
async function handlePost(userId, rawBody) {
  ...
  let topicFeedUrlsFromBody;
  if (topicFeedUrlsRaw !== void 0) {
    if (typeof topicFeedUrlsRaw !== "object" || topicFeedUrlsRaw === null || Array.isArray(topicFeedUrlsRaw)) {
      return json(400, { error: "topicFeedUrls must be an object" });
    }
    const entries = Object.entries(topicFeedUrlsRaw);
    if (entries.length > 20) {
      return json(400, { error: "topicFeedUrls must have at most 20 topics" });
    }
    const parsed = {};
    for (const [topicId, urls] of entries) {
      if (!Array.isArray(urls) || urls.length > 50 || !urls.every((u) => typeof u === "string")) {
        return json(400, {
  ...
  let selectedTopicsFromBody;
  if (selectedTopicsRaw !== void 0) {
    if (!Array.isArray(selectedTopicsRaw) || selectedTopicsRaw.length > 20 || !selectedTopicsRaw.every((t) => typeof t === "string")) {
      return json(400, {
        error: "selectedTopics must be an array of up to 20 strings"
      });
    }
    selectedTopicsFromBody = selectedTopicsRaw;
  }
  const VALID_VOICES = /* @__PURE__ */ new Set([
    "alloy",
    "echo",
    "fable",
    "nova",
  ...
  let durationFromBody;
  if (durationMinutesRaw !== void 0) {
    if (durationMinutesRaw === 3 || durationMinutesRaw === 5 || durationMinutesRaw === 10) {
  ...
  let deliveryHourFromBody;
  if (typeof deliveryHourRaw === "number" && deliveryHourRaw >= 0 && deliveryHourRaw <= 23) {
    deliveryHourFromBody = deliveryHourRaw;
  } else if (deliveryHourRaw !== void 0 && deliveryHourRaw !== null) {
    return json(400, { error: "deliveryHour must be an integer 0\u201323" });
  }
  let digestListenedDatesFromBody;
  if (digestListenedDatesRaw !== void 0) {
    if (!Array.isArray(digestListenedDatesRaw) || digestListenedDatesRaw.length > 31 || !digestListenedDatesRaw.every((d) => typeof d === "string")) {
      return json(400, {
        error: "digestListenedDates must be an array of up to 31 date strings"
      });
    }
    const DATE_RE = /^\d{4}-\d{2}-\d{2}$/;
    if (!digestListenedDatesRaw.every((d) => DATE_RE.test(d))) {
      return json(400, {
        error: "digestListenedDates entries must be YYYY-MM-DD strings"
      });
    }
    digestListenedDatesFromBody = [...new Set(digestListenedDatesRaw)];
  }
  let existingItem;
  try {
  ...
  const mergedTopicFeedUrls = topicFeedUrlsFromBody !== void 0 ? Object.keys(topicFeedUrlsFromBody).length > 0 ? topicFeedUrlsFromBody : null : prev.topicFeedUrls;
  const mergedSelectedTopics = selectedTopicsFromBody !== void 0 ? selectedTopicsFromBody.length > 0 ? selectedTopicsFromBody : null : prev.selectedTopics;
  const mergedVoice = voiceRaw !== void 0 ? voiceFromBody ?? null : prev.voice;
  const mergedDuration = durationMinutesRaw !== void 0 ? durationFromBody ?? null : prev.durationMinutes;
  const mergedDeliveryHour = deliveryHourFromBody ?? prev.deliveryHour;
  const mergedFirstDigestDate = firstDigestDateRaw !== void 0 && typeof firstDigestDateRaw === "string" && !prev.firstDigestDate ? firstDigestDateRaw : prev.firstDigestDate;
  const mergedDigestListenedDates = digestListenedDatesFromBody !== void 0 ? digestListenedDatesFromBody.length > 0 ? digestListenedDatesFromBody : null : prev.digestListenedDates;
  const mergedSubscribed = typeof subscribedRaw === "boolean" ? subscribedRaw : prev.subscribed;
  try {
    await dynamo.send(
      new import_client_dynamodb.PutItemCommand({
        ...
        Item: {
          ...
          ...mergedTopicFeedUrls && Object.keys(mergedTopicFeedUrls).length > 0 ? {
            topicFeedUrls: {
          ...
          ...mergedSelectedTopics && mergedSelectedTopics.length > 0 ? { selectedTopics: { SS: mergedSelectedTopics } } : {},
          ...mergedVoice ? { voice: { S: mergedVoice } } : {},
          ...mergedDuration != null ? { durationMinutes: { N: String(mergedDuration) } } : {},
          ...mergedFirstDigestDate ? { firstDigestDate: { S: mergedFirstDigestDate } } : {},
          ...mergedDigestListenedDates && mergedDigestListenedDates.length > 0 ? { digestListenedDates: { SS: mergedDigestListenedDates } } : {},
...
async function handlePushToken(userId, rawBody) {
  let body;
  try {
    body = JSON.parse(rawBody);
  } catch {
    ...
    try {
      await dynamo.send(
        new import_client_dynamodb.UpdateItemCommand({
          TableName: USERS_TABLE,
          Key: { userId: { S: userId } },
          UpdateExpression: "SET iosPushEnabled = :enabled, updatedAt = :updatedAt REMOVE iosPushToken, iosPushEndpointArn",
  ...
  try {
    const result = await sns.send(
      new import_client_sns.CreatePlatformEndpointCommand({
  ...
  try {
    await sns.send(
      new import_client_sns.SetEndpointAttributesCommand({
...
async function upsertSchedule(userId, timezone, hour2 = 6) {
  const name = `digest-${userId.replace(/[^a-zA-Z0-9-]/g, "_")}`.slice(0, 64);
  const scheduledHour = (hour2 - 1 + 24) % 24;
  const scheduledMinute = pickStableMinute(userId, hour2);
  ...
  try {
    await scheduler.send(new import_client_scheduler.UpdateScheduleCommand(scheduleParams));
  } catch (e) {
    if (e instanceof import_client_scheduler.ResourceNotFoundException) {
      await scheduler.send(new import_client_scheduler.CreateScheduleCommand(scheduleParams));
    } else {
      throw e;
    }
  }
}
function pickStableMinute(userId, hour2) {
  const seed = `${userId}:${hour2}`;
  let hash = 0;
