...
var handler = async (event) => {
  ...
  const VALID_LANGS = ["en", "es", "fr", "de", "pt"];
  ...
  if (pdfBase64) {
    ...
    message2 = { jobId, mode, pdfKey, title, ...voice ? { voice } : {}, ...language ? { language } : {} };
  } else if (url) {
    message2 = { jobId, mode, url, ...voice ? { voice } : {}, ...language ? { language } : {} };
  } else {
    message2 = { jobId, mode, text, title, ...voice ? { voice } : {}, ...language ? { language } : {} };
  }
  await sqs.send(
