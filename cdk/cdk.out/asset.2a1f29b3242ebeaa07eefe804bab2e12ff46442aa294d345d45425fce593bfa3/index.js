...
async function getPresignedDigestAudioUrl(userId, date) {
  return (0, import_s3_request_presigner.getSignedUrl)(
    s3,
    new import_client_s3.GetObjectCommand({ Bucket: BUCKET, Key: digestAudioKey(userId, date) }),
    { expiresIn: 3600 }
  );
}
async function deleteDigestFiles(userId, date) {
  await Promise.allSettled([
    s3.send(new import_client_s3.DeleteObjectCommand({ Bucket: BUCKET, Key: digestStatusKey(userId, date) })),
    s3.send(new import_client_s3.DeleteObjectCommand({ Bucket: BUCKET, Key: digestAudioKey(userId, date) }))
  ]);
}
...
async function handleDeleteToday(requestId, userId, date) {
  console.log("[digest-dispatcher] deleting digest", { requestId, userId, date });
  await deleteDigestFiles(userId, date);
  return json(200, { ok: true });
}
async function handleDispatchDigest(requestId, userId, date, body) {
  const digestId = `${userId}/${date}`;
  let feedUrls;
  if (body.feedUrls !== void 0) {
    if (!Array.isArray(body.feedUrls) || body.feedUrls.length > 50 || !body.feedUrls.every((u) => typeof u === "string")) {
      return json(400, { error: "feedUrls must be an array of up to 50 strings" });
    }
    for (const url of body.feedUrls) {
      try {
        new URL(url);
      } catch {
  ...
  const VALID_VOICES = /* @__PURE__ */ new Set(["alloy", "echo", "fable", "nova", "onyx", "shimmer"]);
  const voice = typeof body.voice === "string" && VALID_VOICES.has(body.voice) ? body.voice : void 0;
  const topN = typeof body.topN === "number" && body.topN > 0 && body.topN <= 20 ? Math.round(body.topN) : void 0;
  let topicFeedUrls;
  if (body.topicFeedUrls !== void 0) {
    ...
    const entries = Object.entries(body.topicFeedUrls);
  ...
  const message2 = { userId, date };
  if (feedUrls && feedUrls.length > 0) message2.feedUrls = feedUrls;
  else if (topicFeedUrls && Object.keys(topicFeedUrls).length > 0) message2.topicFeedUrls = topicFeedUrls;
  if (voice) message2.voice = voice;
  if (topN) message2.topN = topN;
  await sqs.send(
